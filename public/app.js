...
async function api(path,body) {
  if(window.payroomAPI)return window.payroomAPI(path,body);
...
async function run(action,success) {
  if(busy)return;busy=true;$$('button:not(:disabled)').forEach(b=>{b.dataset.requestDisabled='true';b.disabled=true;});notice(window.payroomPublic?'Checking the Sepolia transaction...':'Working on the local chain...');
  try{const result=await action();await refresh();if(result?.state==='failed')notice(result.error||'The chain rejected this change.',true);else if(result?.state==='pending')notice(result.error||'The payment result is still unknown. Reconcile the receipt before retrying.',true);else notice(success);}catch(error){notice(error.message,true);try{await refresh();}catch{}}
...
function renderQueue() {
  ...
  $('#queue-summary').textContent=`${rows.length} ${rows.length===1?'invoice':'invoices'} shown · ${data.config.public?'Sepolia':'Local'} test tokens`;
}
...
function receiptDetails(op,open=false) {
  ...
  return `<details class="receipt-item" ${open?'open':''}><summary>${names[op.kind]||escape(op.kind)} · ${label(op.state)}</summary><dl>${op.receipt?`<dt>Transaction hash</dt><dd>${op.receipt.hash}${data.config.public?`<br><a href="https://sepolia.etherscan.io/tx/${op.receipt.hash}" target="_blank" rel="noopener noreferrer">View on Etherscan</a>`:''}</dd><dt>Block / chain</dt><dd>${op.receipt.blockNumber} / ${op.receipt.chainId} (${data.config.public?'Sepolia':'local'})</dd><dt>Result</dt><dd>${op.receipt.status===1&&!op.receipt.innerFailed?`Confirmed on ${data.config.public?'Sepolia':'the local chain'}`:`Rejected on ${data.config.public?'Sepolia':'the local chain'}`}</dd>`:`<dt>Receipt</dt><dd>${op.hash||'Not found yet. Check the receipt before another attempt.'}</dd>`}${op.error?`<dt>Issue</dt><dd>${escape(op.error)}</dd>`:''}</dl></details>`;
}
...
refresh().catch(error=>notice(window.payroomPublic?`The Sepolia workspace could not load. No payment has been sent. ${error.message}`:`The local workspace is unavailable. Start Payroom and refresh this page. ${error.message}`,true));
